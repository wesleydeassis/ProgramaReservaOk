...
public class OperacoesEnfeite {
	...
	public void CadastrarEnfeites() {   // adicionar no final da lista
		...
		if (inicio == null) {							// verifica se a lista esta vazia
			...
			NO_Enfeite aux = inicio;				// cria endereo de n "aux" e carrega com o endereco de inicio
			while (aux.prox != null) {				// prox vem da classe contructor NO
				aux = aux.prox;						// vai movendo aux para a proximo endereco 
			} // fim while
			NO_Enfeite n = new NO_Enfeite(enfeites);	// cria um novo n com endereco "n" e carrega dado "e"
	...
	public void ListarEnfeites() {
		if (inicio == null) {
			...
			NO_Enfeite aux1 = inicio;	//  de duas variaveis
	...
	public String RemoverInicio() {			// 6 remover no inico da lista
		...
		if (inicio == null) {
			...
			inicio = inicio.prox;			// passar para inicio o ender do proximos endere
		} // fim else
	...
	public String RemoverEnfeites(int pos) {
		...
		if ( pos == 1 ) {  // remoï¿½ï¿½o pos = 1, remoï¿½ï¿½o serï¿½ no inicio da lista
			...
			while (aux.prox != null) {  // remover no final da lista
				aux = aux.prox;   // guarda no aux o endere do proximo da posi
				i++;				// vai guardando os posi ate encontral null
			} // fim While
			...
			else {						// remover qualquer posi
				aux = inicio;			// carrega aux com inicio
				NO_Enfeite aux2 = aux;			// cria endereamenteo aux 2 e copia aux
